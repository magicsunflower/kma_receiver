...
uint8_t USBFS_Endp_DataUp(uint8_t endp, uint8_t *pbuf, uint16_t len, uint8_t mod)
{
    ...
    uint16_t* uep_ctrl;
    uint8_t* uep_dma;
    printf("USBFS_Endp_Busy[%d]=%d\r\n ", endp, USBFS_Endp_Busy[endp]);
    switch (endp) {
        case 1:
                    ...
                    switch( USBFS_SetupReqCode )
                    {
                        ...
                        case USB_GET_DESCRIPTOR:
                            switch( (uint8_t)( USBFS_SetupReqValue >> 8 ) )
                            {
                                ...
                                case USB_DESCR_TYP_DEVICE:
                                    ...
                                    len = USBD_SIZE_DEVICE_DESC;
                                    break;

                                /* get usb configuration descriptor */
                                case USB_DESCR_TYP_CONFIG:
                                    pUSBFS_Descr = USBD_ConfigDescriptor;
                                    len = USBD_SIZE_CONFIG_DESC;
                                    break;
                                ...
                                case USB_DESCR_TYP_HID:
                                    if( USBFS_SetupReqIndex == 0x00 )
                                    {
                                        pUSBFS_Descr = &USBD_ConfigDescriptor[18];
                                        len = 9;
                                    }
                                    else if(USBFS_SetupReqIndex == 0x01)
                                    {
                                        pUSBFS_Descr = &USBD_ConfigDescriptor[43];
                                        len = 9;
                                    }
                                    else
                                    {
                                ...
                                case USB_DESCR_TYP_REPORT:
                                    if( USBFS_SetupReqIndex == 0x00 )
                                    {
                                        pUSBFS_Descr = USBD_KeyRepDesc;
                                        len = USBD_SIZE_REPORT_DESC_KB;
                                    }
                                    else if( USBFS_SetupReqIndex == 0x01 )
                                    {
                                        pUSBFS_Descr = USBD_MouseRepDesc;
                                        len = USBD_SIZE_REPORT_DESC_MS;
                                    }
                                    else
                                    {
                                        errflag = 0xFF;
                                ...
                                case USB_DESCR_TYP_STRING:
                                    switch( (uint8_t)( USBFS_SetupReqValue & 0xFF ) )
                                    {
                                        ...
                                        case DEF_STRING_DESC_LANG:
                                            pUSBFS_Descr = USBD_StringLangDesc;
                                            len = DEF_USBD_LANG_DESC_LEN;
                                            break;

                                        /* Descriptor 1, Manufacturers String descriptor */
                                        case DEF_STRING_DESC_MANU:
                                            pUSBFS_Descr = USBD_ManuInfo;
                                            len = DEF_USBD_MANU_DESC_LEN;
                                            break;

                                        /* Descriptor 2, Product String descriptor */
                                        case DEF_STRING_DESC_PROD:
                                            pUSBFS_Descr = USBD_StringProduct;
                                            len = DEF_USBD_PROD_DESC_LEN;
                                            break;

                                        /* Descriptor 3, Serial-number String descriptor */
                                        case DEF_STRING_DESC_SERN:
                                            pUSBFS_Descr = USBD_StringSerialNum;
                        ...
                        case USB_SET_FEATURE:
                            if( ( USBFS_SetupReqType & USB_REQ_RECIP_MASK ) == USB_REQ_RECIP_DEVICE )
                            {
                                ...
                                if( (uint8_t)( USBFS_SetupReqValue & 0xFF ) == USB_REQ_FEAT_REMOTE_WAKEUP )
                                {
                                    if( USBD_ConfigDescriptor[ 7 ] & 0x20 )
                                    {
